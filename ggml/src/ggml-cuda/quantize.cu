...
static __global__ void quantize_mmq_q4_0(
    ...
    float d;
    if (full_range) {
        const float scale = residual ? 1.0f : amax_scale;
        float d_pos = fmaxf(vmax / 7.0f, -vmin / 8.0f) * scale;
        float d_neg = -fmaxf(vmax / 8.0f, -vmin / 7.0f) * scale;
        if (mse_scale) {
    ...
    if (residual) {
        ...
        const float residual_energy = e0*e0 + e1*e1 + e2*e2 + e3*e3 + e4*e4 + e5*e5 + e6*e6 + e7*e7;
        int rank = 0;
#pragma unroll
        for (int offset = 1; offset < 4; ++offset) {
            const float other_energy = __shfl_xor_sync(0xFFFFFFFF, residual_energy, offset, WARP_SIZE);
            const int other_lane = lane ^ offset;
            rank += other_energy > residual_energy || (other_energy == residual_energy && other_lane < lane);
        }

        residual_mask = rank < 2 ? 1 << lane : 0;
        residual_mask |= __shfl_xor_sync(0xFFFFFFFF, residual_mask, 1, WARP_SIZE);
        residual_mask |= __shfl_xor_sync(0xFFFFFFFF, residual_mask, 2, WARP_SIZE);
        if (!(residual_mask & (1 << lane))) {
            packed_residual = 0;
        }
    }
    ...
    if (scale8) {
        ...
        const float d1 = residual ? (float) residual_mask : (scale16 ? __shfl_xor_sync(0xFFFFFFFF, d, 2, WARP_SIZE) : 0.0f);
        if (lane == 0) {
