...
struct test_mul_mat_w4a4_hadamard : public test_case {
    const int64_t m;
    const int64_t n;
    const int64_t k;
    ...
    test_mul_mat_w4a4_hadamard(int64_t m = 576, int64_t n = 32, int64_t k = 1024) : m(m), n(n), k(k) {
        GGML_ASSERT(k == 1024);
    }
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        ggml_tensor * hx = ggml_mul_mat(ctx, h, x);
        ggml_mul_mat_set_hint(hx, GGML_HINT_SRC0_IS_HADAMARD);
        ggml_set_name(hx, "hx");

        ggml_tensor * w = ggml_new_tensor_2d(ctx, GGML_TYPE_Q4_0, k, m);
        ggml_set_name(w, "w");

        ggml_tensor * out = ggml_mul_mat(ctx, w, hx);
        ggml_set_name(out, "out");
        return out;
    }
...
struct test_mul_mat_w4a4_outliers : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        ggml_tensor * act = x;
        if (randomized) {
            ggml_tensor * signs = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, k);
            ggml_set_name(signs, "signs");
            act = ggml_mul(ctx, act, signs);
            ggml_set_name(act, "dx");
        }
        if (hadamard) {
            ggml_tensor * h = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, k, k);
            ggml_set_name(h, "h");
            act = ggml_mul_mat(ctx, h, act);
    ...
    uint64_t op_flops(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return (hadamard ? 2*k*n*(k + m) : 2*m*n*k) + (randomized ? k*n : 0);
    }
    ...
    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        if (randomized) {
            return "MUL_MAT_W4A4_OUTLIERS_RANDOM_HADAMARD";
        }
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    test_cases.emplace_back(new test_mul_mat_hadamard(GGML_TYPE_F32, GGML_TYPE_F32, 1024, 512, 1024));
    ...
    test_cases.emplace_back(new test_mul_mat_w4a4_outliers(true,  4096, 512, 1024, true));
